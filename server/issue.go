...
func (p *Plugin) UnassignIssue(instance Instance, mattermostUserID types.ID, issueKey string) (string, error) {
	...
	if err != nil {
		return "", errors.Errorf("We couldn't find the issue key `%s`. Please confirm the issue key and try again.", issueKey) //nolint:revive
	}

	if err := client.UpdateAssignee(issueKey, &jira.User{}); err != nil {
		if StatusCode(err) == http.StatusForbidden {
			return "", errors.New("You do not have the appropriate permissions to perform this action. Please contact your Jira administrator.") //nolint:revive
		}
		return "", err
	}
