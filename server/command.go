...
func executeInfo(p *Plugin, c *plugin.Context, header *model.CommandArgs, args ...string) *model.CommandResponse {
	...
	if uinfo.IsConnected {
		...
		resp += sbullet("Name (deprecated)", juser.Name)
		resp += sbullet("Key (deprecated)", juser.Key)
		...
		if uinfo.JIRAUser.Oauth1AccessToken != "" {
			...
			resp += bullet(true, "OAuth1a access secret (length)", len(uinfo.JIRAUser.Oauth1AccessSecret))
		}
	}
