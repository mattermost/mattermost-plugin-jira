	...
	for name, tc := range map[string]struct {
		...
		"JWT instance installed. user is not connected. should return nil user instance": {
			...
			runAssertions: func(p *Plugin, api *plugintest.API) {
				...
				_, _, _, err = p.getClient(types.ID(instanceID), types.ID(mmUserID))
				require.NoError(t, err) // this should return an error but doesn't, since we don't return an error if it doesn't exist in the kv store. doesn't seem to cause any issues with the plugin so maybe its okay
			},
		},
		...
		"migrated JWT to oauth. user is connected to JWT but not oauth. should fetch client for JWT": {
			connection: &Connection{
				User:               jira.User{},
				PluginVersion:      "4.0.1",
				Oauth1AccessToken:  "someaccesstoken",
		...
		"migrated JWT to oauth. user is connected to JWT and oauth. should fetch client for oauth": {
			connection: &Connection{
				User:               jira.User{},
				PluginVersion:      "4.0.1",
				Oauth1AccessToken:  "someaccesstoken",
