  ...
  async () => {
    ...
    try {
      ...
      if (settings.ui_enabled) {
        ...
        registry.registerPostDropdownMenuAction({
          ...
          filter: (postId: string): boolean => {
            ...
            const systemMessage =
              isCombinedUserActivityPost(post) || oldSystemMessageOrNull;
            const installedInstances = getInstalledInstances(state);
      ...
      registry.registerSlashCommandWillBePostedHook(
        hooks.slashCommandWillBePostedHook
      );
    } finally {
      registry.registerWebSocketEventHandler(
        `custom_${PluginId}_connect`,
        handleConnectChange(store)
      );
      registry.registerWebSocketEventHandler(
        `custom_${PluginId}_disconnect`,
        handleConnectChange(store)
      );
      registry.registerWebSocketEventHandler(
        `custom_${PluginId}_update_defaults`,
        handleConnectChange(store)
      );
      registry.registerWebSocketEventHandler(
        `custom_${PluginId}_instance_status`,
        handleInstanceStatusChange(store)
      );
    }
  };
