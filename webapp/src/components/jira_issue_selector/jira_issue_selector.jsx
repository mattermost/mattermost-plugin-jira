    ...
    return this.props
      ...
      .then(({ data }) => {
        return data.map((issue) => {
          const maxLength = 50; // Maximum length for the tooltip text
          const truncatedSummary =
            issue.fields.summary.length > maxLength
              ? `${issue.fields.summary.substring(0, maxLength)}...`
              : issue.fields.summary;
          return {
            value: issue.key,
            label: `${issue.key}: ${truncatedSummary}`,
            title: issue.fields.summary, // Set the full title as the tooltip text
          };
        });
      })
      .catch((e) => {
